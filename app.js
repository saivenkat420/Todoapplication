...
app.post('/todos/', async (request, response) => {
  ...
  if (
    ...
    let addTodoQuery = `insert into todo values(${id},'${todo}','${priority}','${status}','${category}','${
      dueDate[0]
    }-${dueDate[1].padStart(2, '0')}-${dueDate[2].padStart(2, '0')}')`
...
app.put('/todos/:todoId', async (request, response) => {
  ...
  switch (true) {
    ...
    case dueDate !== undefined:
      dueDate = dueDate.split('-')
      let dummyDate = new Date(dueDate[0], dueDate[1] - 1, dueDate[2])
      ...
      if (
        ...
        console.log(dummyDate)
        updateTodoQuery = `update todo set due_date='${dummyDate}' where id=${todoId}`
        await db.run(updateTodoQuery)
